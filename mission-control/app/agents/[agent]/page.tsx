...
import { Send, Loader2, ChevronDown, Copy, Check, Trash2, Wifi, WifiOff, Cpu, Zap, Mic, MicOff } from 'lucide-react'
...
// ─── Canned opening messages per agent ───────────────────────────────────────
  ...
  openclaw: [
    {
      id: 'oc-1',
      role: 'assistant',
      content: 'OpenClaw v2.4.1 initialized. Code analysis engine online.\n\n```\nAST parser: ready\nDependency graph: loaded\nSecurity scanner: active\nJobs in queue: 7\n```\n\nReady to analyze your codebase. Submit a repository path or paste code to begin.',
      timestamp: new Date(Date.now() - 1000 * 60 * 5),
    },
    {
      id: 'oc-2',
      role: 'assistant',
      content: '✓ Completed batch scan — 7 jobs processed. No critical vulnerabilities found. 3 optimization suggestions queued.',
  ...
  hermes: [
    {
      id: 'hm-1',
      role: 'assistant',
      content: 'Hermes routing layer active. 12 message flows currently in transit.\n\n```\nActive routes: 12\nQueued events: 47\nAvg latency: 4ms\nUptime: 99.97%\n```\n\nAll webhooks operational. Monitoring inbound channels.',
      timestamp: new Date(Date.now() - 1000 * 60 * 8),
    },
    {
      id: 'hm-2',
      role: 'assistant',
      content: '⚡ Flow MC-0042 → ATLAS completed in 3.8ms. Routing 12 active event streams across 4 agent endpoints.',
  ...
  atlas: [
    {
      id: 'at-1',
      role: 'assistant',
      content: 'Atlas knowledge base offline. Last sync: 3 hours ago.\n\n```\nVector store: 2.4M embeddings\nIndex status: STALE\nLast query: 3h 12m ago\nConnection: OFFLINE\n```\n\nReconnect to enable knowledge retrieval.',
      timestamp: new Date(Date.now() - 1000 * 60 * 60 * 3),
    },
  ],
}

// ─── Content renderer (supports code blocks) ──────────────────────────────────
    ...
    if (part.startsWith('```') && part.endsWith('```')) {
      ...
      return (
        <div key={i} className="my-2 rounded-lg overflow-hidden border border-white/10">
          ...
          <pre className="p-3 text-xs font-mono text-green-300/80 overflow-x-auto bg-black/20 whitespace-pre-wrap">{code}</pre>
        </div>
      )
    }
...
export default function AgentChatPage({ params }: { params: { agent: string } }) {
  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    if (isClaude) {
      return [{
        id: 'init',
        role: 'assistant',
        content: 'Mission Control initialized. I\'m Claude — ready to assist with any task.\n\n```\nModel: claude-sonnet-4-6\nContext: 200K tokens\nStreaming: enabled\n```',
        timestamp: new Date(),
      }]
    }
  ...
  }, [messages, streamContent, showTyping])
