...
interface FeedEvent { id: string; type: EventType; agent: string; message: string; timestamp: Date }
...
const INITIAL_EVENTS: FeedEvent[] = [
  ...
  { id: '3', type: 'response', agent: 'CLAUDE', message: 'Analysis complete. Found 3 optimization opportunities. Token usage: 847 input, 312 output.', timestamp: new Date(Date.now() - 90000) },
...
export default function ActivityFeed() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      eventIndex.current++
      setEvents(prev => [...prev, { ...template, id: Date.now().toString(), timestamp: new Date() }].slice(-50))
    }, 3500 + Math.random() * 2500)
    return () => clearInterval(id)
  }, [paused])

  useEffect(() => { if (!paused) bottomRef.current?.scrollIntoView({ behavior: 'smooth' }) }, [events, paused])

  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <motion.div animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 1.5, repeat: Infinity }} className="w-2 h-2 rounded-full bg-green-400" style={{ boxShadow: '0 0 6px rgba(34,197,94,0.8)' }} />
          <h3 className="text-xs font-mono font-bold text-white/60 tracking-widest uppercase">Activity Stream</h3>
        </div>
        <button onClick={() => setPaused(p => !p)}
          className={`text-[10px] font-mono px-2 py-0.5 rounded border transition-colors ${paused ? 'text-amber-400 border-amber-500/30 bg-amber-500/10' : 'text-white/30 border-white/10 hover:text-white/60'}`}>
          {paused ? 'PAUSED' : 'LIVE'}
        </button>
      </div>
      <div className="flex-1 overflow-y-auto space-y-1 pr-1">
        <AnimatePresence initial={false}>
          {events.map(event => {
            const cfg = typeConfig[event.type]; const Icon = cfg.icon
            return (
              <motion.div key={event.id} initial={{ opacity: 0, x: -10, height: 0 }} animate={{ opacity: 1, x: 0, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.25 }}
                className="flex items-start gap-2 px-2 py-1.5 rounded-lg group hover:bg-white/3 transition-colors">
                <div className={`flex-shrink-0 w-5 h-5 rounded flex items-center justify-center ${cfg.bg} mt-0.5`}><Icon className={`w-2.5 h-2.5 ${cfg.color}`} /></div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-white/50 font-mono leading-relaxed truncate group-hover:whitespace-normal">{event.message}</p>
                </div>
              </motion.div>
            )
          })}
